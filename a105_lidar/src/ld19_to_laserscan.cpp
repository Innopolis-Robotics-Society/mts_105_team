...
public:
  LD19ToLaserScan() : Node("ld19_to_laserscan") {
    ...
    bins_n_ = std::max(1, int(std::floor((2.0*M_PI) / angle_inc_)));
...
private:
  ...
  bool read_bytes(uint8_t* dst, size_t need, int timeout_ms) {
    size_t got = 0;
    const int chunk_to = std::max(1, timeout_ms / 10);
    auto deadline = std::chrono::steady_clock::now() + std::chrono::milliseconds(timeout_ms);
    while (got < need) {
      if (std::chrono::steady_clock::now() > deadline) return false;
      int n = ::read(fd_, dst + got, int(need - got));
      if (n > 0) got += size_t(n);
      else std::this_thread::sleep_for(std::chrono::milliseconds(chunk_to));
    }
    return true;
  }
  ...
  void publish_scan(const std::vector<std::array<double,3>>& pts) {
    // binning nearest sample per bin
    std::vector<float> ranges(bins_n_, std::numeric_limits<float>::infinity());
    std::vector<float> intens(bins_n_, 0.0f);

    for (const auto& p : pts) {
      double th = p[0], r = p[1]; double inten = p[2];
      if (r < range_min_ || r > range_max_) continue;
      int idx = int(th / angle_inc_);
      if (idx < 0) idx = 0;
      if (idx >= bins_n_) idx = bins_n_-1;
      if (r < ranges[idx]) { ranges[idx] = float(r); intens[idx] = float(inten); }
    }
    ...
    msg.angle_min = 0.0f;
    msg.angle_max = float((bins_n_-1) * angle_inc_);
    msg.angle_increment = float(angle_inc_);
    // scan_time = time since previous publish
    double now_s = now.seconds();
    double scan_time = (last_pub_s_ > 0.0) ? (now_s - last_pub_s_) : 0.0;
    last_pub_s_ = now_s;
    msg.scan_time = float(std::max(0.0, scan_time));
    msg.time_increment = bins_n_ > 0 ? float(msg.scan_time / bins_n_) : 0.0f;
    msg.range_min = float(range_min_);
    msg.range_max = float(range_max_);
    msg.ranges = std::move(ranges);
    msg.intensities = std::move(intens);
  ...
  void reader_loop() {
    ...
    while (running_) {
      ...
      // parse header
      uint16_t speed = uint16_t(frame[2] | (frame[3] << 8));
      (void)speed;
      uint16_t start_angle = uint16_t(frame[4] | (frame[5] << 8));
      // points
      double s_deg = (start_angle % 36000) / 100.0;
      uint16_t end_angle_raw = uint16_t(frame[42] | (frame[43] << 8));
      double e_deg = (end_angle_raw % 36000) / 100.0;
      double diff = std::fmod(e_deg - s_deg + 360.0, 360.0);
      double step = (POINTS_PER_PACK > 1) ? (diff / (POINTS_PER_PACK - 1)) : 0.0;

      // 12 points
      int off = 6;
      for (int i=0;i<POINTS_PER_PACK;i++) {
        uint16_t dist_mm = uint16_t(frame[off] | (frame[off+1] << 8));
        uint8_t inten = frame[off+2];
        off += BYTES_PER_POINT;

        double a_deg = std::fmod(s_deg + i*step, 360.0);
        if (dist_mm == 0 || dist_mm == 0xFFFF || inten == 0) continue;

        double th = transform_angle_rad(a_deg);
        double r_m = dist_mm / 1000.0;

        double deg = std::fmod(a_deg + (clockwise_ ? -this->angle_offset_*180.0/M_PI : this->angle_offset_*180.0/M_PI) + 360.0, 360.0);
        bool wrap = (last_deg >= 270.0 && deg < 90.0);
        if (wrap && !curr.empty()) {
          publish_scan(curr);
          curr.clear();
        }
        curr.push_back({th, r_m, double(inten)});
        last_deg = deg;
      }
    }
  }
...
int main(int argc, char** argv) {
  ...
  try {
    auto node = std::make_shared<LD19ToLaserScan>();
    rclcpp::spin(node);
