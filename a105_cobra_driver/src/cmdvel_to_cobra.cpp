...
public:
  CmdVelToCobra() : Node("cmdvel_to_cobra") {
    ...
    track_      = declare_parameter<double>("track", 0.228);     // м, база колёс
    tpm_        = declare_parameter<double>("tpm", 1000.0);      // ticks per meter
    rate_hz_    = declare_parameter<double>("rate_hz", 30.0);    // частота команд
    poll_hz_    = declare_parameter<double>("poll_hz", 5.0);     // опрос статуса T=130
    deadman_s_  = declare_parameter<double>("deadman_s", 0.3);   // сторож
    ...
    // --- init controller (строго чистый JSON, БЕЗ \\r\\n) ---
    ...
    cmd_timer_ = create_wall_timer(
      std::chrono::milliseconds(int(1000.0 / std::max(1.0, rate_hz_))),
...
private:
  ...
  int open_serial(const char* dev, int baud){
    ...
    tio.c_cc[VMIN]=0;  // nonblocking read with timeout
  ...
  void tick_poll(){
    send_json(std::string("{\"T\":130}")); // запрос статуса
  }

  // Тестовый сценарий: мигаем → вперёд → стоп → назад → повороты
  void test_script(){
    const double t = (now() - test_t0_).seconds();
    if (t < 1.0) { // 0–1 c: мигание 5 Гц
      int on = (int(std::floor(t*10)) & 1) ? 255 : 0;
      char led[64]; std::snprintf(led, sizeof(led), "{\"T\":132,\"IO4\":%d,\"IO5\":%d}", on, on);
      send_json(led);
      send_json(std::string("{\"T\":13,\"X\":0.0,\"Z\":0.0}"));
      return;
    }
    if (t < 3.0) { // 1–3 c: вперёд
      send_json(std::string("{\"T\":132,\"IO4\":255,\"IO5\":255}"));
      send_json(std::string("{\"T\":13,\"X\":0.30,\"Z\":0.0}"));
      return;
    }
    if (t < 3.5){ send_json(std::string("{\"T\":13,\"X\":0.0,\"Z\":0.0}")); return; }
    if (t < 5.5){ send_json(std::string("{\"T\":13,\"X\":-0.30,\"Z\":0.0}")); return; }
    if (t < 6.0){ send_json(std::string("{\"T\":13,\"X\":0.0,\"Z\":0.0}")); return; }
    if (t < 8.0){ send_json(std::string("{\"T\":13,\"X\":0.0,\"Z\":0.8}"));  return; }
    if (t < 10.0){ send_json(std::string("{\"T\":13,\"X\":0.0,\"Z\":-0.8}")); return; }
    send_json(std::string("{\"T\":13,\"X\":0.0,\"Z\":0.0}"));
    send_json(std::string("{\"T\":132,\"IO4\":0,\"IO5\":0}"));
    test_t0_ = now();
  }
  ...
  void rx_loop(){
    ...
    while (rx_run_.load()){
      ...
      // выделяем JSON по балансировке скобок
      size_t i = 0;
      while (i < buf.size()){
  ...
  void handle_json(const std::string& js, rclcpp::Time &last_time){
    ...
    // Основная телеметрия: T=1001, содержит odometry ticks
    if (tcode == 1001){
      int odl=0, odr=0;
      ...
      if (!odo_initialized_){
        ...
        odo_initialized_ = true;
        last_time = tnow;
        return;
      }

      const int dL_ticks = odl - last_left_ticks_;
      const int dR_ticks = odr - last_right_ticks_;
      last_left_ticks_  = odl;
      last_right_ticks_ = odr;

      const double Dl = dL_ticks / tpm_;   // м
      const double Dr = dR_ticks / tpm_;   // м
    ...
    // На T=130/T=12/T=131... просто логируем, если нужно можно расширить
  }
  ...
  rclcpp::TimerBase::SharedPtr cmd_timer_, poll_timer_, test_timer_;
  ...
  bool test_{false};
  rclcpp::Time test_t0_{0,0,RCL_ROS_TIME};
};
...
// ros2 run a105_cobra_driver cmdvel_to_cobra --ros-args -p port:=/tmp/ttyACM1 -p test:=true
