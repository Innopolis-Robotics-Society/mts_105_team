...
class WhiteRoadNode(Node):
    def __init__(self):
        ...
        self.declare_parameters("", [
            ...
            ("cut_robot_enable", True),
            ("cut_robot_y0_frac", 0.62),   # верх выреза (0..1, от верха)
            ("cut_robot_y1_frac", 1.00),   # низ выреза  (0..1)
            ("cut_robot_xc_frac", 0.50),   # центр по X  (0..1)
            ("cut_robot_half_w_frac", 0.24), # половина ширины (0..0.5)
            ...
            ("Ld_min", 0.25), ("Ld_alpha_k", 1.8), ("v_alpha_k", 1.8),
            ("tgt_depth_gain", 1.25),
    ...
    def segment_white(self, bgr, depth_m):
        ...
        # --- вырез прямоугольника робота ---
        if bool(self.get_parameter("cut_robot_enable").value):
            H, W = mask.shape
            y0 = int(clamp(float(self.get_parameter("cut_robot_y0_frac").value) * H, 0, H-1))
            y1 = int(clamp(float(self.get_parameter("cut_robot_y1_frac").value) * H, y0+1, H))
            xc = int(clamp(float(self.get_parameter("cut_robot_xc_frac").value) * W, 0, W-1))
            hw = int(clamp(float(self.get_parameter("cut_robot_half_w_frac").value) * W, 1, W//2))
            x0 = int(max(0, xc - hw))
            x1 = int(min(W, xc + hw))
            mask[y0:y1, x0:x1] = 0
